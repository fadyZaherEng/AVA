          ...
          body: SafeArea(
            child: Column(
              children: [
                ...
                ConditionalBuilder(
                  condition:
                      ChatHomeCubit.get(context).searchLecture.isNotEmpty,
                  builder: (context) => Expanded(
                    child: ListView.separated(
                        physics: const BouncingScrollPhysics(),
                        itemBuilder: (context, index) => buildItem(context,
                            ChatHomeCubit.get(context).searchLecture[index]),
                        separatorBuilder: (context, index) => const SizedBox(
                              height: 15,
                            ),
                        itemCount:
                            ChatHomeCubit.get(context).searchLecture.length),
                  ),
  ...
  Widget buildItem(context, FileModel model) => InkWell(
        ...
        onTap: () async {
          Navigator.push(context,
              MaterialPageRoute(builder: (context) => LectureViewer(model)));
        },
      );
}
