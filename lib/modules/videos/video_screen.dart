...
class VideosScreen extends StatelessWidget {
    ...
    return BlocConsumer<ChatHomeCubit, ChatHomeStates>(
      ...
      builder: (context, state) {
        return ConditionalBuilder(
          condition: ChatHomeCubit.get(context).lecturesVideos.isNotEmpty,
          builder: (context) => ListView.separated(
              physics: const BouncingScrollPhysics(),
              itemBuilder: (context, index) => buildItem(
                  context, ChatHomeCubit.get(context).lecturesVideos[index]),
              separatorBuilder: (context, index) => const SizedBox(
                    height: 15,
                  ),
              itemCount: ChatHomeCubit.get(context).lecturesVideos.length),
  ...
  Widget buildItem(context, VideoModel model) => InkWell(
        child: Card(
          elevation: 15,
        ...
        onTap: () async {
          Navigator.push(context,
              MaterialPageRoute(builder: (context) => VideoWidget(model.link)));
        },
      );
}
