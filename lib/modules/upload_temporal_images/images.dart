...
class UploadImagesUsingUser extends StatelessWidget {
      ...
      builder: (context, state) {
        return Scaffold(
          ...
          body: ConditionalBuilder(
            condition: ChatHomeCubit.get(context).ImagesUsingUser.isNotEmpty,
            builder: (context) => ListView.separated(
                physics: const BouncingScrollPhysics(),
                itemBuilder: (context, index) => buildItem(context,
                    ChatHomeCubit.get(context).ImagesUsingUser[index], index),
                separatorBuilder: (context, index) => const SizedBox(
                      height: 15,
                    ),
                itemCount: ChatHomeCubit.get(context).ImagesUsingUser.length),
  ...
  Widget buildItem(context, ImageModelUser model, index) => Padding(
                  ...
                  child: Row(
                    children: [
                      MaterialButton(
                        color: Colors.green,
                        onPressed: () {
                          ChatHomeCubit.get(context).acceptImageUsingUser(
                              imageId: ChatHomeCubit.get(context)
                                  .ImagesUsingUserId[index]);
                        },
                        child: Text(
                          'Accept',
                          style: Theme.of(context).textTheme.bodyText1,
                      ...
                      MaterialButton(
                        color: Colors.red,
                        onPressed: () {
                          ChatHomeCubit.get(context).rejectImageUsingUser(
                              imageId: ChatHomeCubit.get(context)
                                  .ImagesUsingUserId[index]);
                        },
                        child: Text(
            ...
            Navigator.push(
                ...
                MaterialPageRoute(
                    builder: (context) => ZoomImage(
                        ImageModel(name: model.name, link: model.link))));
          },
        ),
      );
